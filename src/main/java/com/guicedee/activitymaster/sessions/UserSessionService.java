...
public class UserSessionService
    ...
    public Uni<IUserSession<?>> getSession(Mutiny.Session dbSession, IInvolvedParty<?, ?> involvedParty, IUserSession<?> session, ISystems<?, ?> system, java.util.UUID... identityToken) {
        ...
        return involvedParty
                ...
                // A first-time session has no SessionObject resource item — the builder get()
                // throws NoResultException on an empty result, so recover to null to drive the
                // "create new session resource item" branch below.
                ...
                .chain(resourceItem -> {
                    if (resourceItem == null) {
                        ...
                        return dbSession
                                .fetch(resourceItem.getSecondary())
                                .chain(resourceItemFetched -> {
                                    return resourceItemFetched
                                            .getData(dbSession, identityToken)
                                            .chain(data -> {
                                                ...
                                                if (!(sessionString.isEmpty() || "{}".equals(sessionString))) {
                                                    try {
                                                        ...
                                                        log.log(Level.FINE,
                                                                "Error serializing the incoming object to retrieve a session",
                                                                ioException
                                                        );
                                                    }
                                                }
                                                session.setResourceItemID(resourceItemFetched.getId());
                                                session.setDataID(resourceItemFetched.getId());
                                                return Uni
                                                        .createFrom()
                                                        .item(session);
                                            });
                    ...
                    try {
                        return involvedParty
                                ...
                                .chain(relationshipValue -> {
                                    ...
                                    return secondary
                                            .getDataRow(dbSession, identityToken)
                                            .map(data -> {
    ...
    public Uni<IUserSession<?>> expireSession(Mutiny.Session dbSession, IInvolvedParty<?, ?> involvedParty, IUserSession<?> original, ISystems<?, ?> system, java.util.UUID... identityToken) {
        ...
        return resourceItemService
                .findByUUID(dbSession, original.getResourceItemID())
                .chain(resourceItem -> {
                    if (resourceItem != null) {
                ...
                .onFailure()
                .invoke(error -> log.log(Level.SEVERE, "Error expiring session", error))
                .chain(result -> {
                    if (result != null) {
                        return result.setInvolvedParty(involvedParty);
                    }
    ...
    public Uni<IUserSession<?>> updateSession(Mutiny.Session dbSession, IInvolvedParty<?, ?> involvedParty, IUserSession<?> session, ISystems<?, ?> system, java.util.UUID... identityToken) {
        ...
        return Uni
                ...
                .chain(sessionString -> {
                    IResourceItemService<?> resourceItemService = get(IResourceItemService.class);
                    return resourceItemService
                            .updateResourceData(dbSession, sessionString.getBytes(), session.getResourceItemID())
