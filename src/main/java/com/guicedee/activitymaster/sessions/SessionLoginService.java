...
public class SessionLoginService implements ISessionLoginService<SessionLoginService> {
    ...
    @Override
    public Uni<ProfileServiceDTO<?>> loginVisitor(Mutiny.StatelessSession session,
                                                  ProfileServiceDTO<?> profileServiceDTO,
                                                  ISystems<?, ?> system,
                                                  java.util.UUID... identityToken) {
        log.log(Level.FINE,
                "Login visitor (stateless) with web client UUID: {0}",
        ...
        Uni<IInvolvedParty<?, ?>> deviceIPUni = (Uni) iInvolvedParty.builder(session).findByType(TypeDevice.toString(),
                                                                                                 dto.getWebClientUUID()
                                                                                                    .toString(),
                                                                                                 system,
                                                                                                 identityToken).get();
        ...
        return (Uni) deviceIPUni.chain(deviceIP -> {
                                    ...
                                    return updateLatestVisit(session, deviceIP, system, identityToken);
                                }).chain(updatedIP -> sessionMasterService.getUserSession(session, updatedIP, system, identityToken))
                                .map(userSession -> dto).onFailure().invoke(error -> log.log(Level.SEVERE,
                                                                                             "Error in loginVisitor (stateless): {0}",
     ...
     */
    Uni<IInvolvedParty<?, ?>> authenticate(Mutiny.StatelessSession session,
                                           UserLoginDTO<?> loginDTO,
                                           ISystems<?, ?> system,
                                           UUID... identityToken) {
        return passwordsService.findByUsernameAndPassword(session,
     ...
     */
    private Uni<IInvolvedParty<?, ?>> findOrCreateDeviceIP(Mutiny.StatelessSession session,
                                                           ProfileServiceDTO<?> profileServiceDTO,
                                                           ISystems<?, ?> system,
                                                           java.util.UUID... identityToken) {
        log.log(Level.FINE,
                "Finding or creating device IP for web client UUID: {0}",
                profileServiceDTO.getWebClientUUID());
        ...
        return (Uni) involvedPartyService.get().builder(session).findByTypeAll(TypeDevice.toString(),
                                                                               profileServiceDTO.getWebClientUUID()
                                                                                                .toString(),
                                                                               system,
                                                                               identityToken).latestFirst()
                                         .setMaxResults(1).get().onItem().ifNotNull()
                                         .transform(ip -> (IInvolvedParty<?, ?>) ip).onItem().ifNull().switchTo(() -> {
                    // Create new device IP if not found
                    log.log(Level.FINE, "Device IP not found, creating new one via loginVisitor");
                    return (Uni) loginVisitor(session,
                                              profileServiceDTO,
                                              system,
                                              identityToken).chain(dto -> involvedPartyService.get().builder(session)
                                                                                              .findByTypeAll(TypeDevice.toString(),
                                                                                                             profileServiceDTO
                                                                                                                     .getWebClientUUID()
                                                                                                                     .toString(),
                                                                                                             system,
                                                                                              ...
                                                                                              .failWith(() -> new InvolvedPartyException(
                                                                                                      "Device IP must already exist before attempting to login")));
                }).onFailure().invoke(error -> log.log(Level.SEVERE,
                                                       "Error finding or creating device IP: {0}",
                                                       String.valueOf(error)));
    }

    @Override
    public Uni<ProfileServiceDTO<?>> loginUser(Mutiny.StatelessSession session,
                                               UserLoginDTO<?> profileServiceDTO,
                                               boolean alreadyVerified,
                                               ISystems<?, ?> system,
                                               java.util.UUID... identityToken) {
        log.log(Level.FINE,
                "Login user: {0}, already verified: {1}",
                new Object[]{profileServiceDTO.getUserName(), alreadyVerified});
        ...
        return (Uni) withSession(session, dbSession ->
                ...
                findOrCreateDeviceIP(dbSession, profileServiceDTO, system, identityToken).chain(deviceIP -> {
                    // Authenticate user or get involved party
                    Uni<IInvolvedParty<?, ?>> foundPartyUni;
                    if (!alreadyVerified) {
                        foundPartyUni = authenticate(dbSession,
                                                     profileServiceDTO,
                                                     system,
                                                     identityToken).map(foundParty -> {
                            profileServiceDTO.setPassword(null);
                            return foundParty;
                        });
                    } else {
                        foundPartyUni = profileService.findInvolvedParty(dbSession,
                                                                         system,
                                                                         identityToken.length > 0 ? identityToken[0] : null,
                                                                         profileServiceDTO);
                    }
                    ...
                    return foundPartyUni.chain(foundParty -> {
                        ...
                        return deviceIP.findInvolvedPartyIdentificationType(dbSession,
                                       ...
                                       .chain(involvedPartyIdentificationType -> {
                                           ...
                                           if (involvedPartyIdentificationType != null) {
                                               return involvedPartyIdentificationType
                                                       .archive(dbSession, system, identityToken)
                                                       .chain(() -> Uni.createFrom()
                                                                       .item(involvedPartyIdentificationType));
                                           } else {
                                               return Uni.createFrom().nullItem();
                                           }
                                       }).chain(involvedPartyIdentificationType ->
                                                        // Add child
                                                        deviceIP.addChild(dbSession,
                                       ...
                                       .chain(() ->
                                                      // Set user logged in
                                                      setUserLoggedIn(dbSession,
                                                                      foundParty,
                                                                      profileServiceDTO,
                                                                      profileServiceDTO.isRememberMe(),
                                                                      system,
                                                                      identityToken).map(v -> profileServiceDTO));
                    });
                }).onFailure(SecurityAccessException.class).transform(e -> new ProfileServiceException(
                        "Invalid username or password")).onFailure().invoke(error -> log.log(Level.SEVERE,
                                                                                             "Error in loginUser: {0}",
                                                                                             String.valueOf(error))));
    }
    ...
    @Override
    public Uni<ProfileServiceDTO<?>> logoutUser(Mutiny.StatelessSession session,
                                                ProfileServiceDTO<?> profileServiceDTO,
                                                ISystems<?, ?> system,
                                                java.util.UUID... identityToken) {
        // Operate directly on the caller-provided session (this service never opens its own session/tx)
        return withSession(session, dbSession -> {
            UUID token = identityToken.length > 0 ? identityToken[0] : null;
            return profileService.findInvolvedParty(dbSession, system, token, profileServiceDTO).chain(userParty -> {
                // Create device IP
                return createDeviceIP(dbSession, profileServiceDTO, system, identityToken).chain(deviceIP -> {
                    // Find involved party identification type
                    Uni<IRelationshipValue<?, IInvolvedPartyIdentificationType<?, ?>, ?>> findIdentUni;
                    if (userParty != null) {
                        findIdentUni = (Uni) userParty.findInvolvedPartyIdentificationType(dbSession,
                                                                                           NoClassification.toString(),
                                                                                           IdentificationTypeWebClientUUID.toString(),
                                                                                           profileServiceDTO
                                                                                                   .getWebClientUUID()
                        ...
                        findIdentUni = Uni.createFrom().nullItem();
                    }
                    return findIdentUni.chain(idWebClientOpt -> {
                        // Archive if present
                        if (idWebClientOpt != null) {
                            return idWebClientOpt.archive(dbSession, system, identityToken)
                                                 .chain(() -> deviceIP.archiveChild(dbSession,
                        ...
                        return ((IInvolvedParty<?, ?>) devIP).addOrUpdateInvolvedPartyIdentificationType(dbSession,
                                                                                                         NoClassification.toString(),
                                                                                                         IdentificationTypeWebClientUUID.toString(),
                                                                                                         uuid,
                                                                                                         uuid,
                                                             ...
                                                             .chain(idType ->
                                                                            // Set user logged out
                                                                            setUserLoggedOut(dbSession,
                                                                                             userParty,
                                                                                             ((IInvolvedParty<?, ?>) devIP),
                                                                                             profileServiceDTO,
                                                                                             system,
                                                                                             identityToken).map(v -> profileServiceDTO));
                    });
    ...
    @LogItemEvent(Added)
    @Override
    public Uni<Void> setUserLoggedIn(Mutiny.StatelessSession session,
                                     @Party("UserLoggingIn") IInvolvedParty<?, ?> newIp,
                                     @LogItem("SessionObject") ProfileServiceDTO<?> profileServiceDTO,
                                     boolean rememberMe,
                                     @Party("SystemPerformed") ISystems<?, ?> system,
                                     java.util.UUID... identityToken) {
        ...
        UserSecurityDTO us = new UserSecurityDTO().setLoggedIn(true).setRememberMe(rememberMe)
                                                  .setLoginExpiresOn(rememberMe ? LocalDateTime.MAX : com.entityassist.RootEntity
                                                                                                      .getNow()
                                                                                                      .plusMinutes(20));

        // Get session and update it reactively
        return sessionMasterService.getUserSession(session, newIp, system, identityToken).invoke(userSession -> {
            userSession.addValue(IDENTITY_SESSION_NAME, dto);
            userSession.addValue(UserSecurityDTO.USER_SECURITY_SESSION_NAME, us);
            userSession.addValue(USER_ROLES_SESSION_NAME, rolesService.getRoles(session, newIp, system, identityToken));
        }).chain(userSession -> {
            ...
            try {
                IJsonRepresentation.getObjectMapper().readerForUpdating(dto).readValue(dto.toJson());
            } catch (Exception e) {
    ...
    @Override
    public Uni<Void> setUserLoggedOut(Mutiny.StatelessSession session,
                                      @Party("UserLoggedOut") IInvolvedParty<?, ?> involvedParty,
        ...
        // Get session and update it reactively
        Uni<Void> updateSessionUni;
        if (involvedParty != null) {
            updateSessionUni = sessionMasterService.getUserSession(session, involvedParty, system, identityToken)
                                                   .chain(userSession -> {
                                                       // Update session values
                                                       userSession.addValue(UserSecurityDTO.USER_SECURITY_SESSION_NAME,
                                                                            us);
                                                       userSession.removeValue(USER_ROLES_SESSION_NAME);

                                                       // Update session for involved party
                                                       return sessionMasterService.updateSession(session,
                                                                                                 involvedParty,
                                                                                                 userSession,
                                                                                                 system,
                                                                                                 identityToken);
            ...
            updateSessionUni = Uni.createFrom().voidItem();
        }

        return updateSessionUni.chain(() -> {
    ...
    @Override
    public Uni<UserConfirmationKeyDTO<?>> registerVisitor(Mutiny.StatelessSession session,
        ...
        return (Uni) withSession(session, dbSession -> {
            ...
            return involvedPartyService.get().builder(dbSession)
                                       .findByIdentificationType(IdentificationTypeUserName.toString(),
                                                                 userRegistrationDTO.getUserName(),
                                                                 system,
                                                                 identityToken).get().onItem().ifNotNull()
                                       .transformToUni(ipExists -> {
                                           // Check if user has confirmation key
                                           return ipExists.hasClassifications(dbSession,
                                                                              ConfirmationKey,
                                                                              null,
                                                                              system,
                                                                              identityToken)
                                                          .chain(hasConfirmationKey -> {
                                                              if (hasConfirmationKey) {
                                                                  return Uni.createFrom()
                                                                            .failure(new WaitingForConfirmationKeyException(
                                                                                    "The email address is waiting for a confirmation key"));
                                                              } else {
                                                                  return Uni.createFrom()
                                                                            .failure(new UserExistsException(
                                                                                    "That email address is already in use as a valid identifier"));
                                                              }
                                                          });
                                       }).onItem().ifNull().switchTo(() -> {
                        // Create new involved party
                        Pair<String, String> idTypes = new Pair<>(IdentificationTypeUserName.toString(),
                                                                  userRegistrationDTO.getUserName());
                        return involvedPartyService.create(dbSession, system, idTypes, true, identityToken)
                                                   .chain(newIp -> {
                                                       // Expire identification type
                                                       return newIp.findInvolvedPartyIdentificationType(dbSession,
                                                                                                        NoClassification.toString(),
                                                                   ...
                                                                   .chain(idType -> {
                                                                       if (idType != null) {
                                                                           return idType.expire(dbSession,
                                                                                                Duration.of(2, HOURS),
                                                                                                identityToken);
                                                                       }
                                                                       return Uni.createFrom().voidItem();
                                                                   })
                                                                   .chain(() -> passwordsService.addUpdateUsernamePassword(
                                                                           dbSession,
                                                                           userRegistrationDTO.getUserName(),
                                                                           userRegistrationDTO.getPassword(),
                                                                           newIp,
                                                                           system,
                                                                           identityToken)).chain(updatedIp -> {
                                                                   // Clear password for security
                                                                   userRegistrationDTO.setPassword(null);

                                                                   // Find security password classifications
                                                                   return newIp.findClassification(dbSession,
                                                                                                   SecurityPassword,
                                                                                                   system,
                                                                                                   identityToken)
                                                                   ...
                                                                   confirmationKeyDTO.setConfirmationKey(String.valueOf(
                                                                           UUID.randomUUID()));

                                                                   // Add confirmation key classification
                                                                   return newIp.addOrUpdateClassification(dbSession,
                                                                                                          ConfirmationKey,
                                                                                                          (String) null,
                                                                               ...
                                                                               .chain(() -> {
                                                                                   ...
                                                                                   return newIp.findClassification(
                                                                                                       dbSession,
                                                                                                       ConfirmationKey,
                                                                                                       system,
                                                                                                       identityToken)
     ...
     */
    Uni<IInvolvedParty<?, ?>> createDeviceIP(Mutiny.StatelessSession session,
                                             ProfileServiceDTO<?> profileServiceDTO,
                                             ISystems<?, ?> system,
                                             UUID... identityToken) {
        log.log(Level.FINE,
                "Creating device IP (stateless) for web client UUID: {0}",
                profileServiceDTO.getWebClientUUID());
        String webClientUUID = profileServiceDTO.getWebClientUUID().toString();

        return (Uni) withSession(session,
                                 dbSession -> involvedPartyService
                                         .findType(dbSession, TypeDevice.toString(), system, identityToken)
                                         .chain(deviceType -> involvedPartyService.get().builder(dbSession)
                                                                                  .findByTypeAll(TypeDevice.toString(),
                                                                                                 webClientUUID,
                                                                                                 system,
                                                                                                 identityToken)
                                                                                  .latestFirst().setMaxResults(1).get()
                                                                                  ...
                                                                                  .recoverWithNull().onItem()
                                                                                  .ifNotNull()
                                                                                  .transform(ip -> (IInvolvedParty<?, ?>) ip)
                                                                                  .onItem().ifNull().switchTo(() -> {
                                                     log.log(Level.FINE,
                                                             "Device IP not found, creating new one (stateless)");
                                                     ...
                                                     return (Uni) involvedPartyService.create(dbSession,
                                                                                              system,
                                                                                              deviceIDType,
                                                                                              false,
                                                                                              identityToken)
                                                                                      .chain(newIp -> newIp
                                                                                              .addOrReuseInvolvedPartyType(
                                                                                                      dbSession,
                                                                                                      NoClassification.toString(),
                                                                                                      deviceType,
                                                                                                      webClientUUID,
                                                                                         ...
                                                                                         "Error creating device IP (stateless): {0}",
                                                                                         String.valueOf(error))));
    }
     ...
     */
    Uni<IInvolvedParty<?, ?>> updateLatestVisit(Mutiny.StatelessSession session,
        ...
        return (Uni) withSession(session,
                                 dbSession -> newIp.addOrUpdateClassification(dbSession,
                                                                              LastVisitTime,
                                                                              (String) null,
                                                                              lastVisit,
                                                                              system,
                                                                              identityToken)
                                                   .chain(() -> Uni.createFrom().item(newIp)).onFailure()
                                                   .invoke(error -> log.log(Level.SEVERE,
                                                                            "Error updating last visit time (stateless): {0}",
                                                                            String.valueOf(error))));
    }

    @Override
    public Uni<Boolean> verifyUsernameExists(Mutiny.StatelessSession session,
        ...
        return passwordsService.doesUsernameExist(session, userLoginDTO.getUserName(), system, identityToken)
                               .onFailure().invoke(error -> log.log(Level.SEVERE,
                                                                    "Error verifying username: {0}",
                                                                    String.valueOf(error)));
    }

    @Override
    public Uni<UserLoginDTO<?>> verifyPasswordForUser(Mutiny.StatelessSession session,
                                                      UserLoginDTO<?> userLoginDTO,
        ...
        if (Objects.isNull(userLoginDTO.getIdentityToken())) {
            return Uni.createFrom().failure(new ProfileServiceException(
                    "User Login DTO Already needs to have an associated UUID to login with a password"));
        }
     ...
     */
    private static <T> Uni<T> withSession(Mutiny.StatelessSession session,
                                          java.util.function.Function<Mutiny.StatelessSession, Uni<T>> function) {
